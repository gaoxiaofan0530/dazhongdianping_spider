...
class DazhongdianpingSpiderSpiderMiddleware(object):
...
class DazhongdianpingSpiderDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
...
class MyUserAgentMiddleware(UserAgentMiddleware):
    ...
    def process_request(self, request, spider):
        ...
        print('当前USER_AGENT:'+agent)
        request.headers['User-Agent'] = agent

class DangDangWangIPProxy(object):
    # IP代理池中间件
    # 初始化方法
    def __init__(self):
        self.get_url = 'http://api.xedl.321194.com/getip?num=100&type=2&port=1&pack=3363&ts=0&cs=1&lb=1'
        self.temp_url = 'https://www.baidu.com/'
        self.ip_list = []  # 存放代理IP
        self.count = 0  # 记录获取到IP的个数
        self.evecount = 0  # 记录使用次数
